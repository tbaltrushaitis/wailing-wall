 ...
 * Version:\t <%= ME.VERSION %>
 * Commit:\t <%= ME.COMMIT %>
 * Package EOF:\t <%= pkg.name %>@<%= pkg.version %> - <%= pkg.title %>
 * =========================================================================== *
 */
`);
...
gulp.task('build', ['bower', 'sync:src2build'], function () {
  gulp.start('build:assets');
});

gulp.task('build:dev',  ['bower', 'sync:src2build'], function () {
  gulp.start('build:assets');
});
...
gulp.task('bower', function () {
  ...
  let bowerCSS = gulp.src(mBower)
    ...
    .pipe(gulpif('production' === ME.NODE_ENV, cleanCSS(ME.pkg.options.clean, function (d) {
      console.info(d.name + ':\t'    + d.stats.originalSize
                          + '\t->\t' + d.stats.minifiedSize
                          + '\t[' + d.stats.timeSpent + 'ms]'
                          + '\t[' + 100 * d.stats.efficiency.toFixed(2) + '%]');
    }), false))
    .pipe(vinylPaths(function (paths) {
      console.info('CSS:', paths);
      return Promise.resolve();
    }))
    .pipe(gulpif('production' === ME.NODE_ENV, concatCSS('front-bundle.css', {rebaseUrls: false})))
